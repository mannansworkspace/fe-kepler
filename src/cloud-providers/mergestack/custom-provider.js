...
import CustomProviderIcon from "./custom-provider-icon";
...
export default class CustomCloudProvider extends Provider {
  constructor(domain, clientId, apiURL, redirectUri) {
    super({ name: NAME, displayName: DISPLAY_NAME, icon: CustomProviderIcon });
    this.redirectUri = redirectUri;
    this.icon = CustomProviderIcon;
  ...
  async login(onCloudLoginSuccess) {
    ...
    const token = (await this._auth0.isAuthenticated())
      ? await this._auth0.getTokenSilently()
      : null;
  ...
  async uploadMap({ mapData, options = {} }) {
    ...
    const mapResponse = await fetch(`${this.apiURL}/maps/add`, {
      method,
      headers,
